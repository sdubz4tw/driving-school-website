...
export async function GET() {
  ...
  try {
    ...
    if (isBlobConfigured()) {
      console.log(
        "SERVER LOG: Vercel Blob is configured, checking for content.json in blob store"
      );
      ...
      if (contentBlob) {
        console.log(
          "SERVER LOG: Found content.json in blob store, URL:",
          contentBlob.url
        );
        ...
        if (res.ok) {
          ...
          console.log(
            "SERVER LOG: Successfully loaded content from Vercel Blob"
          );
          return NextResponse.json(data);
        }
        console.warn(
          "SERVER LOG: Failed to fetch blob content from URL, falling back to local file"
        );
      } else {
        console.log(
          "SERVER LOG: content.json not found in Vercel Blob yet, falling back to local file"
        );
      }
    } else {
      console.log(
        "SERVER LOG: Vercel Blob not configured, loading from local file"
      );
    }
    ...
    try {
      ...
      console.warn(
        "SERVER LOG: Local file read failed, falling back to hardcoded defaults:",
        fsError
      );
    }

    /* ── 3. Hardcoded baseline defaults (error-intercepting fallback) */
    console.log(
      "SERVER LOG: Returning hardcoded DEFAULT_CONTENT as final fallback"
    );
    return NextResponse.json(DEFAULT_CONTENT);
  } catch (error: unknown) {
    console.error("SERVER LOG: GET request failed:", error);
    // Even on catastrophic failure, return usable defaults so the site renders
    return NextResponse.json(DEFAULT_CONTENT);
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!session || session.value !== "authorized_session_token_value") {
      console.warn(
        "SERVER LOG: Unauthorized access attempt, session cookie missing or invalid"
      );
    ...
    if (!newContent.hero || !newContent.packages || !newContent.contact) {
      console.error(
        "SERVER LOG: Validation failed, structure is missing hero, packages or contact key"
      );
    ...
    if (isBlobConfigured()) {
      console.log(
        "SERVER LOG: Vercel Blob is configured, uploading content.json to Vercel Blob"
      );
      ...
      console.log(
        "SERVER LOG: Vercel Blob upload successful, URL:",
        blob.url
      );
    } else {
      console.log(
        "SERVER LOG: Local filesystem fallback, saving to content.json"
      );
      await fs.writeFile(
        contentFilePath,
    ...
    try {
      ...
      console.warn(
        "SERVER LOG: Non-fatal error during cache revalidation:",
        msg
      );
    }
    ...
    console.error(
      "SERVER LOG: Unhandled exception caught in POST handler:",
      error
    );
