      ...
      <header className="sticky top-0 z-50 w-full backdrop-blur-md border-b transition-all duration-300" style={{ backgroundColor: "color-mix(in srgb, var(--brand-dark, #030712) 92%, transparent)", borderColor: "color-mix(in srgb, var(--brand-light, #1E3E62) 35%, transparent)" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {["Home:#hero","Reviews:#reviews","Contact:#register"].map(l => { const [t,h] = l.split(":"); return <a key={t} href={h} className="text-gray-300 hover:text-white text-sm font-semibold tracking-wide transition-colors">{t}</a>; })}
          </nav>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden px-4 pt-2 pb-6 space-y-2" style={{ backgroundColor: "var(--brand-primary, #0B192C)" }}>
            {["Home:#hero","Reviews:#reviews","Contact:#register"].map(l => { const [t,h] = l.split(":"); return <a key={t} href={h} onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 rounded-lg text-gray-200 hover:text-white font-semibold transition-all">{t}</a>; })}
          </div>
        )}
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-6">
                {[{ icon: MapPin, title: "Service Area", text: contact.address }, { icon: Clock, title: "Hours", text: contact.hours }, { icon: Mail, title: "Email", text: contact.email }].map(({ icon: Icon, title, text }) => (
                  <div key={title} className="flex flex-col items-center text-center p-6 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    <div className="w-12 h-12 rounded-xl text-white flex items-center justify-center flex-shrink-0 mb-4" style={{ backgroundColor: "var(--brand-primary, #0B192C)" }}><Icon size={20} /></div>
